...
const CreateInternationalWeightDisputePopup = ({ open, onClose, onSubmit }) => {
    if (!open) return;
    ...
    const getBoxes = async () => {
        try{
            const response = await fetch(`${API_URL}/order/international/dockets`, {
                method: "POST",
                headers: {
                ...
                body: JSON.stringify({ iid: formData.iid })
            })
            if (!response.ok){
                toast.error("Unable to get order details! Please try again")
            }
            const data = await response.json()
            const boxes = data?.dockets;
            const disputeBoxes = boxes.flatMap((box) =>
                Array(box?.quantity).fill(0).map(() => ({
                    box_no: box?.box_no,
                    docket_weight: box?.docket_weight,
                    docket_weight_unit: box?.docket_weight_unit,
                    length: box?.length,
                    breadth: box?.breadth,
                    height: box?.height,
                    actual_docket_weight: box?.docket_weight,
                    actual_docket_weight_unit: box?.docket_weight_unit,
        ...
        try {
            ...
            await Promise.all(
                Object.entries(files).map(async ([key, file]) => {
                    if (file) {
                        const fileKey = `international_dispute/${formData.iid}/${uuidv4()}`;
            ...
            const response = await fetch(`${API_URL}/weight-disputes/international/create`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': localStorage.getItem('token')
          ...
          <div className="p-6 overflow-y-auto" style={{ maxHeight: 'calc(80vh - 2rem)' }}>
            <form onSubmit={handleSubmit} className="flex flex-col space-y-6 py-8">
                <div className="flex flex-col items-center space-y-2">
                    ...
                    <button type='button' onClick={getBoxes} disabled={!formData?.iid} className="mx-2 px-5 py-1 border rounded-3xl disabled:bg-gray-400 bg-red-500 text-white">Get Order Details</button>
                </div>
                            ...
                            <div className="flex space-x-2">
                                ...
                                <div className="flex gap-1 w-full">
                                    <div className="flex-[2]">
                                        ...
                                        <input key={index} id="actual_docket_weight" name="actual_docket_weight" type="text" value={formData?.dispute_boxes?.[index]?.actual_docket_weight} onChange={(e) => handleBoxChange(e, index)} className="rounded-lg py-1 px-2 w-full" />
                                    </div> 
                                    <div className="flex-1">
                                        <label htmlFor="actual_docket_weight_unit" className="text-sm">Unit</label>
                                        <select key={`actual_docket_weight_unit_${index}`} id="actual_docket_weight_unit" name="actual_docket_weight_unit" value={formData?.dispute_boxes?.[index]?.actual_docket_weight_unit || 'g'} onChange={(e) => handleBoxChange(e, index)} className="rounded-lg py-1 px-2 w-full">
                                            <option value="g">g</option>
                                            <option value="kg">kg</option>
                                        </select>
                                    </div> 
  ...
  export default CreateInternationalWeightDisputePopup;
