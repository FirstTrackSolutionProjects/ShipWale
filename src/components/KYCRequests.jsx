...
const View = ({request, kycId, uid ,fullName, email, phone, gst, setView, businessName, cin, aadhar_number, pan_number, address, city,  state, pin, accountNumber, ifsc, bank}) => {
    const handleApprove = async () => {
        await fetch(`${API_URL}/kyc/request/accept`, {
            method: 'POST',
            headers: { 'Content-Type' : 'application/json',
                'Accept' : 'application/json',
                'Authorization' : localStorage.getItem('token'),
            },
            body: JSON.stringify({uid, kycId})
        }).then(response => response.json()).then(result => alert(result.message));
    }
    const [profilePhoto, setProfilePhoto] = useState(null)
    const handleReject = async () => {
        await fetch(`${API_URL}/kyc/request/reject`, {
            method: 'POST',
            headers: { 'Content-Type' : 'application/json',
                'Accept' : 'application/json',
                'Authorization' : localStorage.getItem('token'),
        ...
        <>
            <div className='absolute inset-0 bg-[rgba(0,0,0,0.5)] z-50 flex justify-center items-center overflow-y-auto'>
                ...
                <p className='absolute top-5 right-6 cursor-pointer' onClick={()=>{setView(false)}}>X</p>
                    <p className='text-2xl font-medium text-center'>KYC Details</p>
                    <div className='w-full space-y-6'>
                        ...
                        <div className='w-full font-medium text-gray-700'>
                            ...
                            <p>CIN : {cin}</p>
                            <p>Aadhar Number : {aadhar_number} <span className='cursor-pointer' onClick={()=>handleDownload('aadharDoc')}>[PDF]</span></p>
                            <p>PAN Number : {pan_number} <span className='cursor-pointer' onClick={()=>handleDownload('panDoc')}>[PDF]</span></p>
                            <p>Address : {address}</p>
                            <p>City : {city}</p>
                            <p>State : {state}</p>
                            <p>Pincode : {pin}</p>
                    ...
                    <button onClick={handleApprove} className=" bg-red-500 text-white mx-2  py-2 px-4 rounded-3xl">
                        Approve
                    </button>
...
const KYCRequests =  () => {
    ...
    useEffect(() => {
        const getVerificationRequests = async () => {
            const response = await fetch(`${API_URL}/kyc/pending/all`, {
                method: 'POST',
  ...
  return (
    ...
    <div className=" py-16 w-full h-full flex flex-col items-center overflow-x-hidden overflow-y-auto">
      ...
      <div className='text-center text-3xl font-medium text-black'>Merchant KYC Requests</div>
