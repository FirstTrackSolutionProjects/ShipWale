...
import getAllInternationalPendingCancellationServices from '../../services/pendingCancellationServices/getAllInternationalPendingCancellationServices';
import approveInternationalCancellationService from '../../services/pendingCancellationServices/approveInternationalCancellationService';
import rejectInternationalCancellationService from '../../services/pendingCancellationServices/rejectInternationalCancellationService';

const InternationalPendingCancellations = () => {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const data = await getAllInternationalPendingCancellationServices();
  ...
  const handleApprove = async (ord_id) => {
    ...
    try {
      await approveInternationalCancellationService(ord_id);
  ...
  const handleReject = async (ord_id) => {
    setActionLoading((prev) => ({ ...prev, [ord_id]: true }));
    try {
      await rejectInternationalCancellationService(ord_id);
...
export default InternationalPendingCancellations;
